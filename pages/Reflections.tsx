...
import { ArrowLeft, Search, Heart, Share2, X, MessageCircle, Sparkles, BookOpen, CheckCircle2 } from 'lucide-react';
...
const Reflections: React.FC = () => {
    ...
    const { isReflectionRead, markReflectionAsRead } = useProgress();
    ...
    return (
        <div className="min-h-screen bg-stone-50 md:bg-stone-100 font-sans pb-20 md:pb-0">
            ...
            <div className="bg-white sticky top-0 z-30 shadow-sm border-b border-stone-100">
                <div className="max-w-4xl mx-auto px-4 py-4 flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] md:text-xs text-amber-600 font-bold mt-1 uppercase tracking-wider flex items-center gap-1">
                ...
                {filteredReflections.map(reflection => (
                    ...
                    >
                        <div>
                            ...
                            <h3 className="text-lg font-bold text-stone-800 mb-2 leading-tight group-hover:text-amber-600 transition-colors flex items-start justify-between gap-2">
                                {reflection.title}
                                {isReflectionRead(reflection.id) && <CheckCircle2 size={18} className="text-emerald-500 flex-shrink-0 mt-1" fill="currentColor" opacity={0.2} />}
                            </h3>
                        ...
                        <div className="mt-4 flex items-center justify-between border-t border-stone-50 pt-4">
                            ...
                            <div className="flex items-center gap-3">
                                {isReflectionRead(reflection.id) && (
                                    <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest bg-emerald-50 px-2 py-0.5 rounded-full">Lido</span>
                                )}
            ...
            {/* Full Screen Study Environment (Folder Style) */}
            {activeReflection && (
                <div className="fixed inset-0 z-[100] bg-stone-900 flex items-center justify-center p-0 md:p-8 animate-in fade-in zoom-in-95 duration-500 overflow-hidden">
                    ...
                    <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-amber-900/20 via-stone-900 to-orange-900/20"></div>
                    ...
                    {/* The "Reflection Folder" Container */}
                    <div className="w-full max-w-6xl h-full md:h-auto md:max-h-[90vh] bg-white md:rounded-[3rem] shadow-[0_30px_120px_rgba(0,0,0,0.6)] relative overflow-hidden flex flex-col md:flex-row animate-in slide-in-from-bottom-12 duration-700 font-sans">
                        ...
                        <div className="flex-1 flex flex-col md:flex-row overflow-y-auto md:overflow-hidden">
                            ...
                            {/* Sidebar: Header & Tags */}
                            <div className="w-full md:w-[35%] bg-amber-600 p-8 md:p-12 text-white flex flex-col relative overflow-hidden flex-shrink-0">
                                <div className="relative z-10 flex flex-col h-full">
                                    <div className="flex items-center gap-4 mb-6 md:mb-8">
                                        ...
                                        <div>
                                            <h2 className="text-3xl md:text-4xl font-serif font-black leading-tight">Reflexão</h2>
                                            <p className="text-amber-100 text-[10px] md:text-sm font-bold uppercase tracking-widest mt-1 opacity-80">Alimento para a alma</p>
                                        </div>
                                    </div>

                                    <div className="flex-1 flex flex-col justify-center py-4 md:py-0">
                                        <h1 className="text-2xl md:text-4xl font-serif font-black mb-6 leading-tight">
                                            {activeReflection.title}
                                        </h1>
                                        <div className="w-16 h-1 bg-white/40 mb-6 font-sans"></div>
                                        <p className="text-amber-100/80 text-sm font-medium italic">
                                            "Sinta a paz neste momento de reflexão profunda."
                                        </p>
                                    </div>

                                    <div className="mt-6 md:mt-8 pt-6 md:pt-8 border-t border-white/20">
                                        <div className="flex gap-2 flex-wrap">
                                            {activeReflection.tags.map(tag => (
                                ...
                                <BookOpen size={200} className="absolute -bottom-10 -right-10 opacity-10 -rotate-12 md:size-[300px] md:-bottom-20 md:-right-20" />
                            </div>

                            {/* Main Content Area: Detailed Content */}
                                ...
                                <div className="max-w-3xl mx-auto space-y-8 md:space-y-12 pb-12 md:pb-0">
                                    ...
                                    <section className="animate-in fade-in slide-in-from-right-8 duration-700 delay-200">
                                        <div className="flex items-center gap-3 mb-6 md:mb-8">
                                            <div className="w-8 h-8 md:w-10 md:h-10 bg-amber-100 text-amber-600 rounded-lg md:rounded-xl flex items-center justify-center shadow-sm">
                                                <Heart size={18} className="md:size-5" />
                                            </div>
                                            <h3 className="text-[10px] md:text-lg font-black uppercase tracking-widest text-stone-400">Sabedoria do Dia</h3>
                                        </div>
                                        <div className="text-lg md:text-2xl text-stone-700 leading-[1.8] font-serif whitespace-pre-line selection:bg-amber-100">
                                            {activeReflection.content}
                                        </div>
                                    </section>

                                    {/* Section: Author */}
                                    {activeReflection.author && (
                                        <section className="bg-amber-50 p-6 md:p-8 rounded-[2rem] border border-amber-100 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-300">
                                            <h4 className="text-[10px] md:text-xs font-black uppercase tracking-widest text-amber-700 mb-2">Autor / Fonte</h4>
                                            <p className="text-stone-700 font-bold font-serif">{activeReflection.author}</p>
                                        </section>
                                    )}
                                    ...
                                    <div className="flex flex-col md:flex-row gap-4 pt-8 pb-10 md:pb-0 border-t border-stone-200">
                                        ...
                                        >
                                            <Share2 size={20} /> Compartilhar Reflexão
                                        </button>
                                        <button
                                            onClick={() => {
                                                markReflectionAsRead(activeReflection.id);
                                                setActiveReflection(null);
                                            }}
                                            className="flex-1 h-12 md:h-14 bg-amber-600 text-white font-black rounded-2xl shadow-xl hover:scale-105 transition-all flex items-center justify-center text-sm md:text-base gap-2"
                                        >
                                            Guardar no Coração {isReflectionRead(activeReflection.id) ? <CheckCircle2 size={20} /> : <Heart size={20} />}
                                        </button>
                                    </div>
                                </div>
                            </div>
