...
export const KidsZone: React.FC = () => {
  ...
  const handleNavClick = (m: 'menu' | 'coloring' | 'quiz' | 'camera' | 'memory' | 'stories' | 'maze' | 'wordsearch') => {
    setMode(m);
  };

  // Render Modes
  if (mode === 'coloring' && coloringImage) {
    ...
    return (
      <ColoringBook
        ...
        onClose={() => handleNavClick('menu')}
      />
    );
  }
  if (mode === 'camera') return <PhotoBooth onBack={() => handleNavClick('menu')} />;
  if (mode === 'quiz') return <KidsQuiz onBack={() => handleNavClick('menu')} />;
  if (mode === 'memory') return <MemoryGame onBack={() => handleNavClick('menu')} />;
  if (mode === 'stories') return <KidsStories onBack={() => handleNavClick('menu')} />;
  if (mode === 'maze') return <MazeGame onBack={() => handleNavClick('menu')} />;
  if (mode === 'wordsearch') return <WordSearch onBack={() => handleNavClick('menu')} />;
